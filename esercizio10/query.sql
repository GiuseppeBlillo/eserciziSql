...
SELECT DISTINCT M.NAME AS MEALS FROM MEAL M
...
SELECT COUNT(DISTINCT M.MEAL_ID) AS NUMBER_OF_VEGETARIAN_MEALS FROM MEAL M
JOIN INGREDIENT I ON M.MEAL_ID = I.MEAL_ID
...
SELECT M.NAME , COUNT(*) AS NUMBER_OF_INGREDIENT FROM MEAL M
...
SELECT DISTINCT M.NAME AS MEALS_WITH_MORE_THAN_3_INGREDIENT FROM MEAL M
JOIN INGREDIENT I ON M.MEAL_ID = I.MEAL_ID
GROUP BY  M.NAME
...
SELECT * FROM CUSTOMER C INNER JOIN O ON C.CUSTOMER_ID = O.CUSTOMER_ID;

-- Using the following table definitions, write an inner join for the two tables

SELECT * FROM INVENTORY I INNER JOIN P ON I.PRODUCT_ID = P.PRODUCT_ID;