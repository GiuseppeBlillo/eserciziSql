...
SELECT MAX (PRICE) FROM MEAL WHERE CALORIES>500;
...
SELECT NAME, PRICE FROM MEAL GROUP BY NAME, PRICE HAVING PRICE = (SELECT MAX(PRICE) FROM MEAL);

-- Write a query to select all meals that have a price lower than the average price, using HAVING and AVG.
SELECT NAME, PRICE FROM MEAL GROUP BY NAME, PRICE HAVING PRICE < (SELECT AVG(PRICE) FROM MEAL);

-- Write a query to select all meals that have a price lower than the average price and have more than 600 calories
SELECT NAME, PRICE FROM MEAL GROUP BY NAME, PRICE HAVING PRICE < (SELECT AVG(PRICE) FROM MEAL) AND MAX(CALORIES)>600;